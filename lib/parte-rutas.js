...
/**
 * Cuánto dura la sesión del tablero. Nico, 10/9: "que la ADMIN_KEY quede
 * guardada en una cookie de 30 días después de ponerla una vez, así desde el
 * celular no la tecleo más".
 *
 ...
 *
 * Se renueva en cada request autorizado, así que abriéndolo una vez por mes no
 * se lo vuelve a pedir nunca.
 */
const DURACION_ADMIN_MS = 30 * 24 * 3600 * 1000;
...
/**
 * Largo mínimo del token del link directo.
 *
 * 🔴 UN TOKEN CORTO EN LA URL ES PEOR QUE UN PIN. Al PIN lo frena el contador de
 * intentos por puesto y por IP; a un link no hay a quién atribuirle los intentos
 * más que a la IP, que se rota. La única defensa es que el espacio sea tan
 * grande que probarlo no tenga sentido: 32 caracteres al azar son más de 10^57
 * combinaciones. Si la variable trae algo más corto, el link NO se habilita.
 */
const LARGO_MINIMO_TOKEN = 32;

/**
 * Un token nuevo, para pegar en Railway.
 *
 * 32 bytes en base64url son 43 caracteres: bien por encima del mínimo, y sin
 * caracteres que se rompan en una URL o que haya que escapar al pegarlos.
 */
function tokenSugerido() {
  return crypto.randomBytes(32).toString('base64url');
}

/** El link directo completo, o null si la variable no habilita ninguno. */
function linkDirecto(base) {
  const tok = String(process.env.PARTE_ADMIN_TOKEN || '');
  if (tok.length < LARGO_MINIMO_TOKEN) return null;
  return String(base || '').replace(/\/+$/, '') + '/parte/admin/t/' + tok;
}

/**
 * Lo que se imprime al arrancar, una sola vez. Nico, 10/9: "imprimir el link
 * completo una sola vez por consola".
 *
 * 🔴 ESTO PONE EL TOKEN EN LOS LOGS DE RAILWAY, Y ESO ES UNA DECISIÓN, NO UN
 * DESCUIDO. El token es la contraseña del tablero. Va igual porque quien puede
 * leer los logs de Railway puede leer las Variables del servicio, que es donde
 * está el token de todos modos: no se le abre la puerta a nadie nuevo. Lo que sí
 * cambia es que el link queda en el historial de deploys, así que si alguna vez
 * se comparte el acceso al proyecto, hay que rotar la variable.
 *
 * Devuelve líneas y no imprime: así se puede probar sin capturar la consola.
 */
function avisoLinkDirecto(base) {
  const tok = String(process.env.PARTE_ADMIN_TOKEN || '');
  if (!tok) {
    return [
      '⚠ PARTE_ADMIN_TOKEN no está seteado: el link directo del tablero NO existe.',
      '  Pegá esta línea en Railway → Variables y volvé a desplegar:',
      '  PARTE_ADMIN_TOKEN=' + tokenSugerido(),
    ];
  }
  if (tok.length < LARGO_MINIMO_TOKEN) {
    return [
      `⚠ PARTE_ADMIN_TOKEN tiene ${tok.length} caracteres y el mínimo son ${LARGO_MINIMO_TOKEN}:`,
      '  el link directo NO existe. Un token corto en la URL es peor que un PIN,',
      '  porque no lo frena ningún contador de intentos. Reemplazalo por:',
      '  PARTE_ADMIN_TOKEN=' + tokenSugerido(),
    ];
  }
  return [
    'Parte · el link del tablero (favoritos, NO a un grupo):',
    '  ' + linkDirecto(base),
  ];
}
...
function crearRutasParte({ secret, adminKey, secure = true }) {
  ...
  /**
   * El acceso directo al tablero. Nico, 8/9: "pasame mi url que yo pueda entrar
   * sin contraseña, acceso directo".
   *
   * 🔴 NO ES "SIN CONTRASEÑA": ES QUE LA CONTRASEÑA SEA EL LINK. El tablero
   * muestra los nombres de todos, la línea de tiempo entera y los montos de la
   * caja. Dejar `/parte/admin` abierto lo pone a la vista de cualquiera, y esa
   * URL se adivina en el primer intento.
   *
   * Con esto: se guarda en favoritos, se toca una vez y entra. El link no se
   * adivina, y se mata cambiando `PARTE_ADMIN_TOKEN` en Railway — sin tocar
   * código y sin invalidar los PIN de la gente.
   *
   * LO QUE HAY QUE SABER: quien tenga el link, entra. No va a un grupo, y si el
   * teléfono se pierde se cambia la variable.
   *
   * Deja la MISMA cookie que el PIN a propósito: un segundo camino de sesión
   * sería un segundo lugar donde equivocarse.
   *
   * Vive en DOS rutas: `/parte/admin/t/<token>`, que es la de ahora y la que
   * queda al lado del tablero, y `/parte/t/<token>`, la del 8/9.
   *
   * 🔴 LA VIEJA NO SE BORRA. Es un link que se guarda en favoritos y se manda
   * por WhatsApp: sacarlo dejaría a Nico afuera del tablero desde el teléfono,
   * sin nada que tocar y sin entender por qué, hasta que alguien le pase el
   * nuevo. Las dos entran a lo mismo; se apagan juntas cambiando la variable.
   */
  async function entrarPorLink(req, res) {
    const esperado = String(process.env.PARTE_ADMIN_TOKEN || '');
    // Sin variable, o con una corta, el link simplemente no existe: que esté
    // vacía no puede querer decir "entra cualquiera".
    if (esperado.length < LARGO_MINIMO_TOKEN) return res.status(404).send('No existe.');

    const dado = String(req.params.token || '');
    const a = Buffer.from(dado);
    const b = Buffer.from(esperado);
    // Largo primero para no reventar en timingSafeEqual, y comparación en tiempo
    // constante: un `===` se corta en el primer carácter distinto.
    if (a.length !== b.length || !crypto.timingSafeEqual(a, b)) {
      // Queda en la línea de tiempo, igual que un PIN errado: si alguien está
      // probando links, se ve.
      await query(
        `INSERT INTO parte_evento (fecha, puesto, tipo, valor) VALUES ($1,$2,'login_fallido',$3)`,
        [fechaOperativa(), PUESTO_ADMIN, huella(ipCliente(req))],
      ).catch(() => {});
    ...
    sembrarSesionAdmin(res);
    res.redirect('/parte/admin');
  }

  r.get('/admin/t/:token', entrarPorLink);
  r.get('/t/:token', entrarPorLink);
...
module.exports = {
  ...
  avisoLinkDirecto, linkDirecto, tokenSugerido, LARGO_MINIMO_TOKEN,
};
