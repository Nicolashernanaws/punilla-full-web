...
// `duracionMs` es parametrizable porque el tablero del dueño dura mucho más que
// un turno: 30 días contra 16 horas. Quien no la pasa sigue teniendo la del
// Parte, que es la que corresponde a la gente que ficha.
function crearToken(datos, secret, ahora = Date.now(), duracionMs = DURACION_MS) {
  const payload = { ...datos, exp: ahora + duracionMs };
