...
async function registrar(datos, db = dbReal) {
  ...
  // 🔴 EL CONSENTIMIENTO YA NO SE EXIGE COMO CAMPO, PERO SE SIGUE GUARDANDO.
  //
  // Se saco el tilde de "acepto las bases": era un paso mas entre el vecino y
  // anotarse, y nadie lo leia. Ahora el aviso esta debajo del boton, a la vista,
  // y apretar "Anotarme" ES la aceptacion.
  //
  // Lo que NO cambia es la prueba: se escribe `consent = true` junto con
  // `creado_en`, asi que queda registrado que acepto y cuando. Dejar de
  // guardarlo seria perder eso, que es lo unico que importa si algun dia hay
  // que mostrar que el que se anoto conocia las bases.
  ...
  return db.withTx(async (client) => {
    ...
    await client.query(
      ...
      // `consent !== false`: si el campo no viene, apretar el boton ya fue la
      // aceptacion. Solo un `false` explicito lo deja en false.
      [telefonoNorm, telefonoRaw || null, nombreLimpio, consent !== false]
    );
